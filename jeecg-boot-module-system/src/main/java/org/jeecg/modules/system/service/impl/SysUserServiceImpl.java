...
import org.jeecg.modules.party_building.mapper.*;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
	...
	public boolean deleteUser(String userId) {
		...
		List<DataAssessment> dataAssessments = dataAssessmentMapper.selectList(new QueryWrapper<DataAssessment>());
		for (DataAssessment dataAssessment:dataAssessments
			 ) {
			String[] split = dataAssessment.getNotAssessMembers().split(",");
			StringBuffer str=new StringBuffer();
			for (String s: split
				 ) {
				boolean equals = s.equals(userId);
				if(equals){
					dataAssessmentMemberMapper.delete(new QueryWrapper<DataAssessmentMember>().eq("assessment_Id",dataAssessment.getId()).eq("assessment_User_Id",s));
					if(dataAssessment.getNotAssessCount()>0){
						dataAssessment.setNotAssessCount(dataAssessment.getNotAssessCount()-1);
					}
					if(dataAssessment.getAssessCount()>0){
			...
			dataAssessment.setNotAssessMembers(str.toString().substring(0,str.length()-1));
			dataAssessmentMapper.updateById(dataAssessment);
		}
		deleteDataActivity(userId);
		return false;
	}
	//删除用户所在活动
	public  void deleteDataActivity(String userId){
		List<DataActivity> dataActivities = dataActivityMapper.selectList(new QueryWrapper<DataActivity>());
		for (DataActivity dataActivity: dataActivities
			 ) {
			String[] split = dataActivity.getPartyMember().split(",");
			StringBuffer str=new StringBuffer();
			for (String id: split
				 ) {
				boolean equals = id.equals(userId);
				if(equals){
					dataActivitySignUpMapper.delete(new QueryWrapper<DataActivitysignUp>().eq("activity_Id",dataActivity.getId()).eq("user_Id",id));
					dataActivitySignMapper.delete(new QueryWrapper<DataActivitySign>().eq("actity_Sign_Up_Id",dataActivity.getId()).eq("user_Id",id));
				}
				if(!equals){
					str.append(id + ",");
				}
			}
			dataActivity.setPartyMember(str.toString().substring(0,str.length()-1));
