...
import org.jeecg.modules.party_building.util.PwdVerificationUtil;
...
public class SysUserController {
	...
	public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
        ...
        String[] split = selectedDeparts.split(",");
        if(split.length>1){
            return result.error500("一个人只允许有一个部门");
        }
        try {
	...
	public Result<SysUser> edit(@RequestBody JSONObject jsonObject) {
		...
		try {
			...
			if(sysUser==null) {
                ...
                if(departs!=null){
                    String[] split = departs.split(",");
                    if(split.length>1){
                        return result.error500("一个人只允许有一个部门");
                    }
                }
                if(roles==null || departs==null){
    ...
    @RequestMapping(value = "/queryById1", method = RequestMethod.GET)
    public Result<JSONObject> queryById1(@RequestParam(name = "id", required = true) String id) {
	...
	public Result<?> changPassword(@RequestBody JSONObject json) {
        ...
        boolean flag=PwdVerificationUtil.isLetterDigit(password);
		if (!flag){
            return Result.error("密码必须包含数字和字母且长度在8-16位");
        }
