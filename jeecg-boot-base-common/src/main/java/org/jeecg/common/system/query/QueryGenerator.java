...
public class QueryGenerator {
	 ...
	 * @return QueryWrapper实例
	 */
	public static <T> QueryWrapper<T> initQueryWrapper(T searchObj,Map<String, String[]> parameterMap,String table){
		long start = System.currentTimeMillis();
		QueryWrapper<T> queryWrapper = new QueryWrapper<T>();
		installMplus(queryWrapper, searchObj, parameterMap,table);
		log.debug("---查询条件构造器初始化完成,耗时:"+(System.currentTimeMillis()-start)+"毫秒----");
		return queryWrapper;
	}
	/**
	 ...
	 */
	public static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap,String table) {
		...
		*/
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				...
				Object obj = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getAttribute(name + END);
				if(Objects.nonNull(obj)){
					endValue = obj.toString();
					addQueryByRule(queryWrapper, name, type, endValue, QueryRuleEnum.LE);
				}
				//多值查询
				...
				if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
					...
					if(StringUtils.isBlank(table)){
						...
						addEasyQuery(queryWrapper, table+"."+name, rule, value);
